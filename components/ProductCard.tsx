...
const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart, isInWishlist, onToggleWishlist, onSelectProduct }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden transition-transform duration-300 hover:scale-105 hover:shadow-xl flex flex-col relative group">
       <button 
        onClick={() => onToggleWishlist(product.id)}
        className="absolute top-2 left-2 z-10 p-2 bg-white/70 rounded-full text-red-500 hover:bg-white transition-colors"
      ...
      >
        ...
        <div className="p-4 flex flex-col flex-grow">
          <h3 className="text-lg font-semibold text-slate-800 group-hover:text-blue-600 transition-colors">{product.name}</h3>
